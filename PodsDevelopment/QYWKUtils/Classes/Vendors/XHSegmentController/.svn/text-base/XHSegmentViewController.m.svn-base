...
#define DefaultSegmentHeight 50
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = YYBCUtilsRGB(246, 246, 246, 1);
...
- (XHSegmentControl *)segmentControl
{
    if (!_segmentControl)
    {
        _segmentControl = [[XHSegmentControl alloc] initWithFrame:CGRectMake(0,15, [UIScreen mainScreen].bounds.size.width, DefaultSegmentHeight)];
...
- (UIScrollView *)scrollView
{
    ...
    if (!_scrollView)
    {
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_segmentControl.frame), [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - CGRectGetMaxY(_segmentControl.frame) - YYBCUtilsiPhoneXBottomSafeFieldHeight)];
...
- (void)setSegmentControlWidth:(CGFloat)segmentControlWidth
{
    _segmentControlWidth = segmentControlWidth;
    CGRect frame = self.segmentControl.frame;
    frame.size.width = segmentControlWidth;
    self.segmentControl.frame = frame;
}

-(void)setSegmentControlY:(CGFloat)segmentControlY
{
    _segmentControlY = segmentControlY;
    CGRect frame = self.segmentControl.frame;
    frame.origin.y = segmentControlY;
    self.segmentControl.frame = frame;
    self.scrollView.frame = CGRectMake(0, CGRectGetMaxY(_segmentControl.frame), [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - CGRectGetMaxY(_segmentControl.frame) - YYBCUtilsiPhoneXBottomSafeFieldHeight);
}
...
-(void)setHasBottomLine:(BOOL)hasBottomLine
{
    ...
    self.segmentControl.hasBottomLine = hasBottomLine;
