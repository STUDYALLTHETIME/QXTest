...
//    [self createItems];
    ...
    if (self.segmentType == XHSegmentTypeFilled) {
        for (XHSegmentItem *item in self.items) {
            CGRect frame = item.frame;
            frame.size.width = size.width / self.items.count;
            item.frame = frame;
        }
    }
    ...
    if (self.hasBottomLine) {
        ...
        line.frame = CGRectMake(0, CGRectGetHeight(self.frame) - self.margin, [[UIScreen mainScreen] bounds].size.width, 0.8);
    ...
    if (self.hasTopLine) {
        UIView *line = [[UIView alloc]init];
        line.backgroundColor = [UIColor colorWithRed:241/255.0 green:241/255.0 blue:241/255.0 alpha:1];
        line.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, 0.8);
        [self addSubview:line];
    }
...
- (void)load
{
    ...
    //  load 高亮线
    self.lineLayer.backgroundColor = self.highlightColor.CGColor;
     self.lineLayer.frame = CGRectMake(CGRectGetMinX(self.lineLayer.frame), CGRectGetHeight(self.frame) - self.margin, CGRectGetWidth(self.lineLayer.frame), self.lineWidth);
...
- (void)setSelectIndex:(NSInteger)selectIndex animation:(BOOL)animation
{
    ...
    if (selectIndex < self.items.count) {
        ...
        CGFloat W = 30;
        self.lineLayer.frame = CGRectMake(CGRectGetMinX(item.frame) + (CGRectGetWidth(item.frame) - 30) / 2, CGRectGetHeight(item.frame) - self.margin, W, self.lineWidth);
